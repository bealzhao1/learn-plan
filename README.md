# learn-plan

技术学习路线文档，涵盖 Go 语言全栈、前端 React、数据库、中间件、大模型开发等核心知识点。

## 目录结构

```
docs/
├── go/                            # Go 语言相关
│   ├── 01-go-core.md              # Go 语言核心
│   ├── 02-go-compiler.md          # Go 编译原理
│   ├── 03-go-build-tools.md       # Go 打包工具
│   ├── 04-go-gmp.md               # GMP 调度模型
│   ├── 05-go-concurrency.md       # Go 并发编程
│   └── 06-go-channel.md           # Channel 深入解析
├── web/                           # Web 框架与设计模式
│   ├── 07-gin-framework.md        # Gin Web 框架设计原理
│   └── 08-design-patterns.md      # 设计模式的优点
├── react/                         # React 前端
│   ├── 21-react-core.md           # React 核心与渲染机制
│   ├── 22-react-hooks.md          # Hooks 深入
│   ├── 23-react-state.md          # 状态管理
│   ├── 24-react-router-data.md    # 路由与数据请求
│   ├── 25-react-performance.md    # 性能优化
│   └── 26-react-ssr-nextjs.md     # SSR 与 Next.js
├── mysql/                         # MySQL 数据库
│   ├── 09-mysql-index.md          # MySQL 索引
│   └── 10-mysql-transaction.md    # MySQL 事务
├── redis/                         # Redis 缓存
│   ├── 11-redis-data-structures.md # Redis 数据结构
│   └── 12-redis-scenarios.md      # Redis 场景分析与使用
├── mq/                            # 消息队列
│   ├── 13-kafka-scenarios.md      # Kafka 场景分析与使用
│   ├── 14-kafka-vs-pulsar.md      # Kafka vs Pulsar 对比
│   ├── 29-mq-compensation.md      # 消息队列补偿机制
│   ├── 30-kafka-hw-mechanism.md   # Kafka HW 高水位机制
│   └── 31-kafka-consumer-lag.md   # Kafka 消息堆积排查与治理
├── k8s/                           # Kubernetes 容器编排
│   ├── 16-k8s-architecture.md     # K8s 架构与核心概念
│   ├── 17-k8s-workloads.md        # 工作负载与部署策略
│   ├── 18-k8s-networking.md       # 网络与服务治理
│   ├── 19-k8s-storage.md          # 存储与配置管理
│   └── 20-k8s-ops.md              # 安全与运维实践
├── gateway/                       # 网关与限流降级
│   └── 27-gateway-nginx-architecture.md  # Nginx + 自研网关（限流+降级）
├── observability/                 # 可观测性
│   └── 28-prometheus-observability.md    # Prometheus 与全链路监控
└── ai/                            # 大模型与智能体
    └── 15-llm-agent-dev.md        # 大模型 MCP / Skills / 工作流 / 智能体开发
```

## 适用场景

- 技术面试复习
- 日常开发参考
- 新人 onboarding 学习路线

## 技术栈

| 领域 | 技术 |
|------|------|
| 语言 | Go |
| 前端 | React / Next.js |
| Web 框架 | Gin |
| 数据库 | MySQL |
| 缓存 | Redis |
| 消息队列 | Kafka / Pulsar |
| 容器编排 | Kubernetes |
| 网关 | Nginx + 自研网关 |
| 可观测性 | Prometheus / Loki / Jaeger |
| AI 开发 | MCP Server / Agent |

## License

MIT
